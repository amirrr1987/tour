<template>
  <UCard>
    <UForm :schema="schema" :state="state" class="space-y-4" @submit.prevent="onSubmit">
      <UFormField label="درجه سختی" name="tourLevelType" :ui="formGroupUi">
        <URadioGroup
          v-model="props.state.tourLevelType"
      ...
      <UFormField name="transferType" :ui="formGroupUi">
        ...
        <URadioGroup
          v-model="props.state.transferType"
      ...
      <UDivider />
      <div class="flex">
        <UButton size="xs" type="submit">اعمال فیلتر</UButton>
        <UButton size="xs" type="" variant="ghost" @click="onReset">پاک کردن</UButton>
      </div>
    </UForm>
  </UCard>
</template>
