...
import type { IFilter } from '~/models/filter.model'
  ...
  const handleSearch = () => {
    ...
    if (formData.value.search) {
      query.search = formData.value.search
    }
    if (formData.value.location) {
      query.location = formData.value.location
    }
    if (formData.value.startDate) {
      query.startDate = formData.value.startDate
    }
      ...
      <UContainer>
        <div class="text-center max-w-4xl mx-auto">
          ...
          <p class="text-xl text-muted mb-8 max-w-2xl mx-auto">
            Explore the world with our carefully curated tours. From adventure to relaxation, we
      ...
      <UCard class="bg-linear-to-r from-primary/10 to-primary/5">
        <div class="text-center py-12">
          ...
          <p class="text-lg text-muted mb-8 max-w-2xl mx-auto">
            Join thousands of satisfied travelers and discover the world with us. Browse our tours
            or get in touch to plan your perfect trip.
          </p>
          <div class="flex flex-col sm:flex-row gap-4 justify-center">
            <UButton to="/tours" size="lg" icon="i-lucide-map"> Explore Tours </UButton>
